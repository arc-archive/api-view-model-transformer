  ...
  [
    ...
    describe(item[0], () => {
      ...
      function computePropertyShape(element, amf, declareIndex) {
        ...
        if (element._hasType(dec, ns.raml.vocabularies.shapes + 'ScalarShape')) {
          return dec;
        }
        const key = element._getAmfKey(ns.w3.shacl.name + 'property');
      ...
      describe('_uiModelForPropertyShape()', () => {
        ...
        it('Has name and binding only when no range', () => {
          ...
          const key = element._getAmfKey(ns.raml.vocabularies.shapes + 'range');
          delete model[key];
